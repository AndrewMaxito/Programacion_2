...
void muestraCamiones(void *flota,int numCamiones,void (*imprimeDatos)(ofstream &,void *),
    ...
    //REcorrer Pila (solo se puede recorrer para imprimir -> el problema lo dice sino desapilar uno a uno)
    void **regFlota = (void **)flota;
    for (int i = 0; i < numCamiones; i++) {
        arch<<left << setw(10) <<"Camion:"<< i+1 << setw(7) << ":";
        imprimeDatos(arch,regFlota[i]);
    }
}