...
void operator>>(ifstream &arch, Plato &plato) {
    ...
    if (arch.eof()) {
        strcpy_s(plato.codigo,sizeof("FIN"), "FIN"); //le pone un delimitador
        return;
    }
...
void operator>>(ifstream& arch, Repartidor& repartidor) {
    ...
    if (arch.eof()) {
        strcpy_s(repartidor.codigo, sizeof("FIN"), "FIN");
        return;
    }
...
void ingresarOrdenCompra(OrdenDeCompra &ordenesDeCompra, const Pedido &ped) {
    ...
    if (posPlato == NO_ENCONTRADO) {
        ...
        strcpy_s(ordenesDeCompra.platosSolicitados[posPlato].codigo,sizeof(ped.codigoDelPlato), ped.codigoDelPlato);
        ordenesDeCompra.platosSolicitados[posPlato].precio = ped.precio;
        ordenesDeCompra.platosSolicitados[posPlato].cantidad = 0; //Si es nuevo el plto debe ser incializado en 0
